...
export async function GET(req: Request) {
  ...
  const m = await getMemberByInvite(token);
  if (!m) return NextResponse.json({ valid: false }, { status: 404 });
  return NextResponse.json({ valid: true, name: m.name, role: m.role, email: (await displayFor(m)).email });
}
